...
export default function Profissionais({ avisar, tratarErro }) {
  ...
  const carregar = useCallback(() => {
    api('barbeiros').then((r) => setItens(r.itens)).catch(tratarErro);
  }, [tratarErro]);

  useEffect(carregar, [carregar]);

  async function salvar() {
    if (!editando.nome.trim()) return avisar('Escreva o nome do profissional.', 'erro');
    setSalvando(true);
    try {
      if (editando.id) {
        await api(`barbeiros/${editando.id}`, { method: 'PATCH', body: editando });
      } else {
        await api('barbeiros', {
          method: 'POST',
  ...
  async function alternarAtivo(item) {
    try {
      await api(`barbeiros/${item.id}`, { method: 'PATCH', body: { ativo: item.ativo ? 0 : 1 } });
      carregar();
      avisar(item.ativo ? `${item.nome} não aparece mais no site.` : `${item.nome} voltou ao site.`);
    } catch (erro) {
      tratarErro(erro);
    }
  }
  ...
  return (
    <>
      <div className="conteudo-topo">
        ...
        <button className="btn btn-ouro" onClick={() => setEditando({ ...VAZIO })}>
      ...
      {itens.length === 0 ? (
        <Vazio titulo="Nenhum profissional cadastrado">
          Cadastre a equipe para que os clientes possam escolher com quem se atender.
        </Vazio>
      ) : (
        <div className="cartoes" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(300px,1fr))' }}>
          {itens.map((b) => (
            <article className="cartao" key={b.id}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 13, color: 'var(--tinta-suave)' }}>{b.funcao}</div>
                </div>
                <span className={`etiqueta ${b.ativo ? 'etiqueta-confirmado' : 'etiqueta-neutra'}`}>
                  {b.ativo ? 'No ar' : 'Fora'}
                </span>
              </div>

              {b.bio ? (
                <p style={{ margin: '0 0 14px', fontSize: 14, color: 'var(--tinta-suave)' }}>
                  {b.bio}
                </p>
              ) : null}

              <div className="acoes-linha">
                <button className="btn btn-contorno btn-mini" onClick={() => setEditando(b)}>
                  <Lapis width={14} height={14} /> Editar
                </button>
                <button className="btn btn-contorno btn-mini" onClick={() => alternarAtivo(b)}>
                  {b.ativo ? 'Tirar do site' : 'Voltar ao site'}
                </button>
                <button className="icone-btn perigo" onClick={() => excluir(b)} title="Excluir">
