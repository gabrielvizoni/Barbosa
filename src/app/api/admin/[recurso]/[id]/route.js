...
export const PATCH = comLog(
  ...
  async (request, { params }) => {
    ...
    const auditarPreco =
      params.recurso === "servicos" && "preco_centavos" in campos;
    const conn = getDb();
...
export const DELETE = comLog(
  ...
  async (request, { params }) => {
    ...
    if (params.recurso === "barbeiros" || params.recurso === "servicos") {
      const coluna =
        params.recurso === "barbeiros" ? "barbeiro_id" : "servico_id";
      const usos = getDb()
      ...
      if (usos > 0) {
        const resultado = getDb()
          .prepare(`UPDATE ${recurso.tabela} SET ativo = 0 WHERE id = ?`)
          .run(id);
        if (resultado.changes === 0) {
          return Response.json(
    ...
    const resultado = getDb()
      .prepare(`DELETE FROM ${recurso.tabela} WHERE id = ?`)
      .run(id);
    if (resultado.changes === 0) {
      return Response.json({ erro: "Item não encontrado." }, { status: 404 });
    }
