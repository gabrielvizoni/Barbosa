...
![Testes](https://img.shields.io/badge/testes-186%20passando-2e7d32)
...
> self-contained SQLite rate limiter — no bcrypt, no Redis, no NextAuth. 186 tests on Node's
...
| Testes automatizados            | **186**, no runner nativo do Node — zero framework de teste            |
...
| Testes       | **runner nativo do Node** (`node --test`) | 186 testes, sem Jest nem Vitest.                                                                                   |
...
186 testes no runner embutido do Node, cobrindo a lógica que não pode quebrar:
...
tests/                          186 testes no runner nativo do Node
```
