...
| Rotinas (agendador externo) | **`[PARCIAL]`** — `POST /api/tarefas/lembretes` · `POST /api/tarefas/marcar-no-show` · `POST /api/tarefas/limpeza` já existem e exigem o **segredo** em header (`X-Tarefa-Segredo`), não sessão (RNF-22); a lógica de cada rotina ainda é um stub (`{ ok: true, processados: 0 }`) até os epics que a implementam |
