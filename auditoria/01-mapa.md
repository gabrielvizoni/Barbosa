...
| *(implícitos)* | PK de `config(chave)`, `expediente(dia)`, `schema_version` — sem; `servico_barbeiro(servico_id, barbeiro_id)`; rowid das tabelas AUTOINCREMENT | — | — |
...
9. **`POST /api/agendamentos` (público) responde 200**, o painel responde
   201. Os testes checam `resultado.ok` do objeto de `criarAgendamento`, não
