...
4. **Soft delete que mantém a linha para sempre** (Etapa 8 do plano). É o
   ...
   *plano de retenção* para a `auditoria`; não desfaz o soft delete.
   **Manter.**
...
7. **F3 — `usos` não filtra `excluido_em`.** O efeito é **conservador na
   ...
   *desativado* em vez de apagado fisicamente — que é provavelmente o
...
`ok: true` (201), **não** 409. **Variante:** com um agendamento *excluído* às
...
| *(fora do código)* | Script de backup consistente + runbook de restauração + restore testado. | **F30** |
...
- **(a)** Exigir `TRUST_PROXY=1` + proxy reverso real em produção (nginx/
  Caddy/Cloudflare) e voltar ao limite por IP. *É a solução mais limpa;
  precisa que você controle o proxy.*
