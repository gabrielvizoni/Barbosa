...
test("público: telefone é obrigatório, com 10-11 dígitos", () => {
  const semTelefone = criarAgendamento(
    base({ origem: "publico", clienteTelefone: "" }),
  );
  assert.equal(semTelefone.ok, false);
  assert.equal(semTelefone.status, 400);

  const curto = criarAgendamento(
    base({ origem: "publico", clienteTelefone: "4499999" }),
  );
  assert.equal(curto.ok, false);

  const valido = criarAgendamento(
    base({ origem: "publico", clienteTelefone: "44999998888" }),
  );
...
test("painel: telefone é opcional, mas validado quando informado", () => {
  ...
  const telefoneRuim = criarAgendamento(
    base({ clienteTelefone: "123", inicio: "11:00" }),
  );
...
test("painel nunca encaixa em cima de outro atendimento do mesmo profissional", () => {
  const primeiro = criarAgendamento(
    base({ clienteNome: "Primeiro", inicio: "10:00" }),
  );
  assert.equal(primeiro.ok, true);

  const segundo = criarAgendamento(
    base({ clienteNome: "Segundo", inicio: "10:15" }),
  );
