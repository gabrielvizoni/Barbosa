...
import { validar, dataValida, horaValida, validarExpediente, primeiroErro } from '../src/lib/validacao.js';

test('rejeita duracao_min = 0 na criação de serviço', () => {
  const { ok, erros } = validar('servicos', { nome: 'Corte', duracao_min: 0 }, { criando: true });
  assert.equal(ok, false);
  assert.ok(erros.duracao_min);
});

test('aceita duracao_min dentro da faixa (5 a 480)', () => {
  assert.equal(validar('servicos', { nome: 'Corte', duracao_min: 5 }, { criando: true }).ok, true);
  assert.equal(validar('servicos', { nome: 'Corte', duracao_min: 480 }, { criando: true }).ok, true);
  assert.equal(validar('servicos', { nome: 'Corte', duracao_min: 481 }, { criando: true }).ok, false);
});

test('rejeita preco_centavos negativo e acima do teto de 10.000.000', () => {
  assert.equal(validar('servicos', { nome: 'x', preco_centavos: -1 }).ok, false);
  assert.equal(validar('servicos', { nome: 'x', preco_centavos: 10_000_001 }).ok, false);
  assert.equal(validar('servicos', { nome: 'x', preco_centavos: 10_000_000 }).ok, true);
});
...
test('primeiroErro formata "campo: mensagem", ou null sem erros', () => {
  assert.equal(primeiroErro({ nome: 'é obrigatório.' }), 'nome: é obrigatório.');
