...
import { horariosLivres, agora, somarDias, diaDaSemana } from '../src/lib/slots.js';
...
test('agendamento que termina fora da grade libera o horário logo em seguida', () => {
  ...
  const livres = horariosLivres({ barbeiroId: barbeiro1, duracaoMin: 15, data: SEGUNDA });
  assert.deepEqual(livres, ['09:45']);
});

test('bloqueio com barbeiro_id nulo afeta todos os profissionais', () => {
  definirExpediente(1, { aberto: 1, abre: '09:00', fecha: '10:00' }, conn);
  criarBloqueio({ barbeiroId: null, data: SEGUNDA, inicio: '09:00', fim: '10:00' }, conn);
  assert.deepEqual(horariosLivres({ barbeiroId: barbeiro1, duracaoMin: 30, data: SEGUNDA }), []);
  assert.deepEqual(horariosLivres({ barbeiroId: barbeiro2, duracaoMin: 30, data: SEGUNDA }), []);
});

test('bloqueio com barbeiro_id específico só afeta aquele profissional', () => {
  definirExpediente(1, { aberto: 1, abre: '09:00', fecha: '10:00' }, conn);
  criarBloqueio({ barbeiroId: barbeiro1, data: SEGUNDA, inicio: '09:00', fim: '10:00' }, conn);
  assert.deepEqual(horariosLivres({ barbeiroId: barbeiro1, duracaoMin: 30, data: SEGUNDA }), []);
  assert.deepEqual(horariosLivres({ barbeiroId: barbeiro2, duracaoMin: 30, data: SEGUNDA }), [
    '09:00',
    '09:30',
...
test('antecedencia_min corta os horários de hoje que já passaram do limite, mas não os de amanhã', () => {
  ...
  definirExpediente(hoje.diaSemana, { aberto: 1, abre: '00:00', fecha: '23:59' }, conn);
  definirExpediente(diaSemanaAmanha, { aberto: 1, abre: '00:00', fecha: '23:59' }, conn);
  salvarConfig({ antecedencia_min: '100000' }); // maior que qualquer janela do dia

  const livresHoje = horariosLivres({ barbeiroId: barbeiro1, duracaoMin: 30, data: hoje.data });
  const livresAmanha = horariosLivres({ barbeiroId: barbeiro1, duracaoMin: 30, data: amanha });

  assert.deepEqual(livresHoje, []);
  assert.ok(livresAmanha.length > 0, 'amanhã não deveria sofrer o corte de antecedência de hoje');
});

test('serviço com duração maior que o expediente devolve lista vazia', () => {
  definirExpediente(1, { aberto: 1, abre: '09:00', fecha: '10:00' }, conn);
  const livres = horariosLivres({ barbeiroId: barbeiro1, duracaoMin: 90, data: SEGUNDA });
  assert.deepEqual(livres, []);
});
