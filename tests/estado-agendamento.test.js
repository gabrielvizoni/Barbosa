...
import { criarAgendamento, mudarStatusAgendamento, remarcarAgendamento } from '../src/lib/agendamentos.js';
...
beforeEach(() => {
  ...
  definirExpediente(diaDaSemana(DATA_FUTURA), { aberto: 1, abre: '09:00', fecha: '20:00' }, conn);
...
function criar(overrides = {}) {
  ...
  assert.equal(r.ok, true, `fixture deveria ter criado com sucesso: ${JSON.stringify(r)}`);
  return r.id;
}
...
test('transição concluido→pendente é rejeitada', () => {
  ...
  conn.prepare("UPDATE agendamentos SET status = 'concluido', data = ? WHERE id = ?").run(agora().data, id);
...
test('reabertura de cancelado é rejeitada quando o horário já foi ocupado por outro agendamento', () => {
  ...
  const linha = conn.prepare('SELECT status FROM agendamentos WHERE id = ?').get(id);
...
test('remarcação para horário ocupado é rejeitada', () => {
  ...
  const linha = conn.prepare('SELECT inicio FROM agendamentos WHERE id = ?').get(idAlvo);
  assert.equal(linha.inicio, '16:00', 'remarcação rejeitada não deveria ter mudado nada');
});
...
test('remarcação de agendamento concluído ou cancelado é rejeitada', () => {
  ...
  conn.prepare("UPDATE agendamentos SET status = 'cancelado' WHERE id = ?").run(id);
