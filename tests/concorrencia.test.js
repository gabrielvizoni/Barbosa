...
const CAMINHO_WORKER = fileURLToPath(
  new URL("./worker-criar-agendamento.mjs", import.meta.url),
);
...
test("dois agendamentos concorrentes no mesmo horário: um vence (201), o outro recebe 409 — nunca os dois", async () => {
  ...
  assert.equal(
    sucessos.length,
    1,
    "exatamente um dos dois deveria ter criado o agendamento",
  );
  ...
  assert.equal(
    total,
    1,
    "não pode existir mais de um agendamento ativo no mesmo horário",
  );
});
