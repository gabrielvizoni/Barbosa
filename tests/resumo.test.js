...
beforeEach(async () => {
  conn = bancoDeTeste();
  conn.exec('DELETE FROM agendamentos; DELETE FROM barbeiros; DELETE FROM servicos;');
  __resetCookies();
  // Senha própria já cadastrada, para não cair na trava da senha inicial.
  await salvarConfig({ senha_hash: await gerarHash('senha-de-teste-99'), sessao_versao: '1' });
...
function inserirAgendamento({
  ...
  conn
    ...
    .run(barbeiroId, servicoId, barbeiroNome, servicoNome, data, inicio, fim, precoCentavos, status);
}
...
test('renomear o profissional entre dois agendamentos não parte o relatório em duas linhas', async () => {
  ...
  conn.prepare('UPDATE barbeiros SET nome = ? WHERE id = ?').run('Ana Donegá', barbeiroId);
  ...
  const resposta = await GET(
    new Request('http://localhost/api/admin/resumo?mes=2025-03', { method: 'GET' })
  );
...
test('os totais mensais da série batem com a soma dos cartões, para a mesma definição', async () => {
  ...
  const somaDosCartoes = financeiro.principal.realizado.faturamento + financeiro.principal.previsto.faturamento;
